...
bot=telebot.TeleBot('7057456584:AAHZyyIzouiooceM74LGHlbTHaGPeZvHnJQ')
    ...
    if request.method == 'POST':
        ...
        for t in user_cart:
            ...
            text += (f'Товар:{t.user_product}\n'
                     ...
                     f'Цена: ${round(t.user_product * t.user_pr_count), 2}'
                     f'-------------------------------\n')
        text += f'Итого:${round(sum(total),2)}'
        bot.send_message(7708088534,text)
        user_cart.delete()
        return redirect('/')
